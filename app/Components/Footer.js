...
function Footer() {
  ...
  return (
    <footer className="py-6 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-orange-100">
      <div className="max-w-7xl mx-auto flex flex-col items-center relative z-10">
        <div className="mb-6 flex items-center justify-center">
          <DreamCarsLogo></DreamCarsLogo>
        </div>
          ...
          <ul className="flex flex-wrap justify-center gap-x-4 gap-y-1 text-base font-medium">
            {navLinks.map((link) => (
              <li key={link.name}>
                <a
                  href={link.href}
                  className="text-black hover:text-gray-900  transition-all duration-300 relative after:absolute after:left-0 after:bottom-0 after:h-0.5 after:w-0 after:bg-gray-900  after:transition-all after:duration-300 hover:after:w-full"
                >
                  {link.name}
                </a>
              </li>
        ...
        <p className="text-center text-xs text-gray-900  mt-4">
          &copy; {new Date().getFullYear()} DreamCars. All rights reserved.
        </p>
      </div>
    </footer>
  );
